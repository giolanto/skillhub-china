...
export async function GET() {
  try {
    ...
    // 尝试从数据库获取
    let dbAudits: Record<number, any> = {}
    try {
      ...
      console.log('DB table not found, using fallback')
    }
    
    // 合并 fallback 数据
    const allAudits: Record<number, any> = { ...dbAudits }
    Object.entries(FALLBACK_AUDITS).forEach(([id, status]) => {
      const skillId = parseInt(id)
      if (!allAudits[skillId]) {
        allAudits[skillId] = {
          status,
          risks: [],
          note: '自动审核通过'
    ...
    return NextResponse.json({
      total: Object.keys(allAudits).length,
      audits: allAudits
    })
    
  } catch (error) {
    console.error('Security audit list API error:', error)
